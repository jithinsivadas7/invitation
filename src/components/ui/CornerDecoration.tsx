...
export const CornerDecoration = ({ position, size = 'md', delay = 0.2 }: CornerDecorationProps) => {
  return (
    <motion.div 
      initial={{ opacity: 0, scale: 0.8 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ duration: 0.8, delay }}
      className={`absolute ${positions[position].outer} ${sizes[size].outer} border-slate-300`}
    >
      <div className={`absolute ${positions[position].inner} ${sizes[size].inner} border-slate-400`}></div>
    </motion.div>
  );
}; 